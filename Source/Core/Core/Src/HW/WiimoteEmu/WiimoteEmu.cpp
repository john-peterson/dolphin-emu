...
//	, m_sound_stream( NULL )
{
	...
	// swing
	groups.push_back(m_thrust = new Force(_trans("Thrust")));

	// tilt
	groups.push_back(m_rotate = new Rotate(_trans("Rotate"), true));
...
void Wiimote::GetAccelData(u8* const data, u8* const buttons)
{
	...
	// ----TILT----
	EmulateTilt(&m_accel, m_rotate, has_focus, is_sideways, is_upright, m_ir, GetMotionPlusActive(), m_options->settings[SETTING_IR_OFF]->value != 0);
